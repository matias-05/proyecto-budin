...
export default function CartContainer() {
  ...
  const confirmDelete = () => {
    if (productToDelete) {
      removeItem(productToDelete.id, productToDelete.precioUnidad);
    ...
    const detallePedido = finalItems
      .map((item) => {
        ...
        if (item.agregados && item.agregados.length > 0) {
          const listaAgregados = item.agregados
            .map((a) => `   └ ${a}`)
            .join("\n");
          textoItem += `\n${listaAgregados}`;
        }
      ...
      <section className="min-h-screen bg-[#e37b00] pt-32 flex justify-center px-4 font-['Dosis']">
        <div className="bg-[#681104] p-8 md:p-12 rounded-3xl shadow-2xl text-center h-fit max-w-lg border border-white/10 animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-white text-lg mb-8">
            Tu pedido se registró como pago por{" "}
            <strong className="text-[#e37b00]">{metodoPagoTexto}</strong>.{" "}
            <br /> Para finalizar, presioná el botón y envianos el mensaje.
          </p>
  ...
  return (
    <section className="min-h-screen bg-[#e37b00] pt-28 pb-20 px-4 md:px-12 font-['Dosis'] relative">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
                      ...
                      <div className="flex items-center gap-2 md:gap-4 bg-black/30 p-1.5 rounded-xl border border-white/5">
                        ...
                        >
                          {item.quantity === 1 ? (
                            ...
                            <Minus size={16} className="md:w-6 md:h-6" />
                          )}
                        </button>
          ...
          <div className="lg:w-1/3">
            <div className="bg-[#681104] rounded-3xl p-8 shadow-2xl border border-white/5 sticky top-28">
              <h2 className="text-[#e37b00] text-3xl font-black uppercase italic mb-6 border-b border-[#e37b00]/20 pb-4 text-center">
                Resumen
              </h2>
              <div className="flex justify-between text-white text-xl mb-4">
                <span className="font-bold opacity-70 italic">Productos:</span>
                <span className="font-black text-[#e37b00]">
                  {countCartItems()}
                </span>
              </div>
              <div className="flex justify-between text-white text-3xl mb-8 items-center border-t border-[#e37b00]/20 pt-6">
                ...
                <span className="font-black text-[#e37b00] tracking-tighter">
                  ${totalPrice()}
                </span>
              </div>
          ...
          <div className="bg-[#681104] border-2 border-[#e37b00] p-8 rounded-3xl shadow-2xl max-w-sm w-full relative z-10 animate-in zoom-in slide-in-from-bottom-4 duration-300 text-center">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="bg-[#e37b00] text-[#681104] font-black py-4 rounded-2xl uppercase tracking-widest hover:bg-white transition-colors"
              >
                Sí, eliminar
              </button>
              <button
